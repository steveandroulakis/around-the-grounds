...
class TestBaseParser:
    ...
    @pytest.fixture
    def parser(self, sample_brewery):
        """Create a parser instance for testing."""
        return ConcreteParser(sample_brewery)
    
    def test_parser_initialization(self, sample_brewery):
    ...
    @pytest.mark.asyncio
    async def test_fetch_page_404_error(self, parser):
    ...
    @pytest.mark.asyncio
    async def test_fetch_page_403_error(self, parser):
    ...
    @pytest.mark.asyncio
    async def test_fetch_page_500_error(self, parser):
    ...
    @pytest.mark.asyncio
    async def test_fetch_page_empty_response(self, parser):
    ...
    @pytest.mark.asyncio
    async def test_fetch_page_network_error(self, parser):
        """Test handling of network errors."""
        with aioresponses() as m:
            m.get("https://example.com/test", exception=aiohttp.ClientError("Network error"))
            
            async with aiohttp.ClientSession() as session:
                with pytest.raises(ValueError, match="Network error fetching"):
                    await parser.fetch_page(session, "https://example.com/test")
    
    def test_validate_event_valid(self, parser, sample_food_truck_event):
        """Test validation of valid events."""
        result = parser.validate_event(sample_food_truck_event)
        assert result is True
    
    def test_validate_event_missing_brewery_key(self, parser, sample_food_truck_event):
        """Test validation with missing brewery key."""
        sample_food_truck_event.brewery_key = ""
        result = parser.validate_event(sample_food_truck_event)
        assert result is False
    
    def test_validate_event_missing_brewery_name(self, parser, sample_food_truck_event):
        """Test validation with missing brewery name."""
        sample_food_truck_event.brewery_name = ""
        result = parser.validate_event(sample_food_truck_event)
        assert result is False
    
    def test_validate_event_missing_food_truck_name(self, parser, sample_food_truck_event):
        """Test validation with missing food truck name."""
        sample_food_truck_event.food_truck_name = ""
        result = parser.validate_event(sample_food_truck_event)
        assert result is False
    
    def test_validate_event_missing_date(self, parser, sample_food_truck_event):
        """Test validation with missing date."""
        sample_food_truck_event.date = None
        result = parser.validate_event(sample_food_truck_event)
        assert result is False
    
    def test_filter_valid_events(self, parser, sample_food_truck_event):
    ...
    def test_filter_valid_events_empty_list(self, parser):
        """Test filtering empty list of events."""
        filtered_events = parser.filter_valid_events([])
        assert filtered_events == []
    
    def test_filter_valid_events_all_invalid(self, parser):
        """Test filtering when all events are invalid."""
        invalid_event1 = FoodTruckEvent("", "Brewery", "Truck", None)
        invalid_event2 = FoodTruckEvent("key", "", "Truck", None)
    ...
    @pytest.mark.asyncio
    async def test_fetch_page_non_html_response(self, parser):
    ...
    @pytest.mark.asyncio
    async def test_fetch_page_malformed_html(self, parser):
