...
class TestVisionAnalyzer:
    ...
    def test_is_valid_image_url(self, vision_analyzer):
        ...
        assert vision_analyzer._is_valid_image_url("https://images.example.com/photo.png")
    ...
    @pytest.mark.asyncio
    @patch('anthropic.Anthropic')
    async def test_analyze_image_with_suffix_cleaning(self, mock_anthropic, vision_analyzer):
    ...
    async def test_analyze_image_api_error(self, mock_anthropic, vision_analyzer):
        ...
        result = await vision_analyzer.analyze_food_truck_image("https://example.com/test.jpg")
        assert result is None
    
    @pytest.mark.asyncio
    @patch('around_the_grounds.utils.vision_analyzer.VisionAnalyzer._analyze_image_by_url')
    async def test_analyze_image_retry_logic(self, mock_analyze, vision_analyzer):
        ...
        result = await vision_analyzer.analyze_food_truck_image("https://example.com/test.jpg", max_retries=1)
        assert result == "Georgia's"
    ...
    @pytest.mark.asyncio
    @patch('around_the_grounds.utils.vision_analyzer.VisionAnalyzer._analyze_image_by_url')
    async def test_analyze_image_max_retries_exceeded(self, mock_analyze, vision_analyzer):
    ...
    @pytest.mark.asyncio
    @patch('around_the_grounds.utils.vision_analyzer.VisionAnalyzer._analyze_image_by_url')
    async def test_analyze_image_general_exception_handling(self, mock_analyze, vision_analyzer):
        # Mock any kind of exception
        mock_analyze.side_effect = RuntimeError("General API error")
        
        # Should handle gracefully
        result = await vision_analyzer.analyze_food_truck_image("https://example.com/test.jpg", max_retries=0)
        assert result is None
