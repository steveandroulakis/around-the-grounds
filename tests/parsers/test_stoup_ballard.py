...
class TestStoupBallardParser:
    ...
    def test_parse_time_invalid_hour(self, parser):
        ...
        date = datetime(2025, 7, 5)
        start_time, end_time = parser._parse_time(date, (13, 8, "pm"))  # 13pm doesn't exist
    ...
    async def test_parse_network_error(self, parser):
        ...
        with aioresponses() as m:
            ...
            async with aiohttp.ClientSession() as session:
                with pytest.raises(ValueError, match="Failed to parse Stoup Ballard website"):
    ...
    async def test_parse_fallback_extraction(self, parser):
        ...
        with aioresponses() as m:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                assert len(events) >= 0  # May or may not find events depending on parsing logic
