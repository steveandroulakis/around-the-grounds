...
class ScrapingError:
    ...
    def __init__(self, brewery: Brewery, error_type: str, message: str, details: str = None):
...
class ScraperCoordinator:
        ...
        async with aiohttp.ClientSession(
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    ...
                    self.logger.error(f"Unexpected error scraping {breweries[i].name}: {result}")
                    continue
    ...
    async def _scrape_brewery_with_error_handling(self, session: aiohttp.ClientSession, brewery: Brewery) -> List[FoodTruckEvent]:
        """
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt == self.max_retries - 1:
                    ...
                    self.logger.warning(f"Timeout scraping {brewery.name}, retrying in {wait_time}s...")
                ...
                if attempt == self.max_retries - 1:
                    ...
                    self.errors.append(error)
                    self.logger.error(f"Network error scraping {brewery.name}: {error_msg}")
                    return []
                else:
                    wait_time = 2 ** attempt
                    self.logger.warning(f"Network error scraping {brewery.name}, retrying in {wait_time}s...")
                    await asyncio.sleep(wait_time)
                ...
                if attempt == self.max_retries - 1:
                    ...
                    self.errors.append(error)
                    self.logger.error(f"Unknown error scraping {brewery.name}: {error_msg}")
                    return []
                else:
                    wait_time = 2 ** attempt
                    self.logger.warning(f"Unknown error scraping {brewery.name}, retrying in {wait_time}s...")
                    await asyncio.sleep(wait_time)
